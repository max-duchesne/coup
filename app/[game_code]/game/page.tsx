...
function formatLogEntry(event: GameEvent, players: GameState["players"]): string {
  ...
  switch (event.action) {
    ...
    case "lose_influence": {
      const roleLabel = event.metadata?.role
        ? ROLE_LABELS[event.metadata.role as keyof typeof ROLE_LABELS] ?? event.metadata.role
        : "an influence";
...
export default function GamePage() {
  ...
  const refreshGameState = useCallback(async () => {
    ...
    if (!state) { router.replace(`/${gameCode}`); return; }
    setGameState(state);
  }, [gameCode, router]);
  ...
  useEffect(() => {
    ...
    const dbChannel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "games", filter: `game_code=eq.${gameCode}` }, () => { void refreshGameState(); })
      .on("postgres_changes", { event: "*", schema: "public", table: "game_players", filter: `game_code=eq.${gameCode}` }, () => { void refreshGameState(); })
      .on("postgres_changes", { event: "*", schema: "public", table: "player_influences", filter: `game_code=eq.${gameCode}` }, () => { void refreshGameState(); })
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "game_events", filter: `game_code=eq.${gameCode}` }, () => { void refreshLog(); })
  ...
  useEffect(() => {
    ...
    presenceChannel
      ...
      .on("presence", { event: "join" }, ({ key, newPresences }) => {
        ...
        setConnectionLog((prev) => [...prev, { id: crypto.randomUUID(), message: `${name} connected.`, ts: Date.now() }]);
      })
      .on("presence", { event: "leave" }, ({ key, leftPresences }) => {
        flushOnlineIds();
        if (key === playerId) return;
        const name = (leftPresences[0] as unknown as PresencePayload | undefined)?.name ?? "A player";
        setConnectionLog((prev) => [...prev, { id: crypto.randomUUID(), message: `${name} disconnected.`, ts: Date.now() }]);
      })
      .subscribe(async (next) => {
        setPresenceStatus(next as ConnectionStatus);
        if (next === "SUBSCRIBED") await presenceChannel.track({ id: playerId, name: playerName });
      });
  ...
  const mergedLog = useMemo(() => {
    ...
    const connEntries = connectionLog.map((e) => ({ key: `conn-${e.id}`, message: e.message, ts: e.ts }));
  ...
  const wrap = async (fn: () => Promise<void>) => {
    ...
    try { await fn(); }
    catch (err) { showError(err instanceof Error ? err.message : "Action failed"); }
    finally { setActionPending(false); }
  };

  if (!playerId) return <main style={{ padding: 24, fontFamily: "sans-serif" }}>Loading…</main>;

  if (!gameState) {
    return (
      <main style={{ padding: 24, fontFamily: "sans-serif" }}>
        <p>Connecting to game…</p>
        <p style={{ color: "#666", fontSize: 13 }}>db: <code>{dbStatus}</code> · presence: <code>{presenceStatus}</code></p>
      </main>
    );
  }

  const { currentTurnPlayerId, turnPhase, pendingTargetId, players } = gameState;
  const me = players.find((p) => p.playerId === playerId);
  const currentTurnPlayer = players.find((p) => p.playerId === currentTurnPlayerId);
  const isMyTurn = currentTurnPlayerId === playerId;
  const iMustLoseInfluence = turnPhase === "lose_influence" && pendingTargetId === playerId;
  ...
  return (
    <main style={{ padding: 24, fontFamily: "sans-serif" }}>
      ...
      <section style={{ marginTop: 20 }}>
        ...
        <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
          {players.map((p) => {
            ...
            const isTurn = p.playerId === currentTurnPlayerId;
            const isMe = p.playerId === playerId;
            return (
              <li key={p.playerId} style={{ padding: "6px 0", fontWeight: isTurn ? "bold" : "normal", color: isOnline ? "inherit" : "#999" }}>
                {p.name}{isMe ? " (you)" : ""} — {p.coins} coin{p.coins !== 1 ? "s" : ""}
                {isTurn ? " ← current turn" : ""}
                {!isOnline ? " (offline)" : ""}
                {/* Influences */}
                <ul style={{ listStyle: "none", padding: "4px 0 0 16px", margin: 0, fontSize: 13 }}>
                  {p.influences.map((inf) => (
                    <li key={inf.id} style={{ color: inf.isRevealed ? "#c00" : "inherit" }}>
                      {isMe || inf.isRevealed
      ...
      <section style={{ marginTop: 24 }}>
        {iMustLoseInfluence ? (
          <>
            <p style={{ fontWeight: "bold", marginBottom: 10 }}>You have been couped — choose an influence to lose:</p>
            <div style={{ display: "flex", gap: 10 }}>
              {myLiveInfluences.map((inf) => (
                <button
                  ...
                  onClick={() => void wrap(() => loseInfluence(gameCode, playerId, inf.id))}
                >
          ...
          <p style={{ color: "#666" }}>
            Waiting for {players.find((p) => p.playerId === pendingTargetId)?.name ?? "opponent"} to choose an influence…
          </p>
        ) : isMyTurn ? (
          <>
            {mustCoup && (
              <p style={{ color: "crimson", marginBottom: 8 }}>You have 10+ coins — you must Coup!</p>
            )}
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button
                ...
                onClick={() => void wrap(() => takeAction(gameCode, playerId, "income"))}
              >
                Take Income (+1)
              </button>
              <button
                type="button"
                disabled={actionPending || mustCoup}
                onClick={() => void wrap(() => takeAction(gameCode, playerId, "foreign_aid"))}
              >
                Take Foreign Aid (+2)
              </button>
              {/* Coup buttons — one per opponent */}
              {players
                ...
                .map((target) => (
                  <button
                    ...
                    onClick={() => void wrap(() => performCoup(gameCode, playerId, target.playerId))}
                  >
          ...
          <p style={{ color: "#666" }}>Waiting for {currentTurnPlayer?.name ?? "other player"}…</p>
        )}
      </section>
      ...
      {mergedLog.length > 0 && (
        <section style={{ marginTop: 24 }}>
          ...
          <ol style={{ paddingLeft: 20, margin: 0 }}>
            {mergedLog.map((entry) => (
              <li key={entry.key} style={{ marginBottom: 4 }}>{entry.message}</li>
            ))}
          </ol>
        </section>
      )}
